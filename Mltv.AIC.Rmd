...
p_values <- coef(sumwgtgest)[,"p-value"]

# Apply Benjamini-Hochberg procedure to control FDR at 
adjusted_p_values <- p.adjust(p_values, method = "BH", n = length(p_values))

tenPercent<-adjusted_p_values < 0.1

fivePercent<-adjusted_p_values < 0.05
```
...
Recut Data for adult weight + gest mal
```{r}
...
Weight + gestation
```{r}
wgtgest<-pglsSEyPagel(MalignancyPrevalence~log10(adult_weight.g.) +log10(Gestation.months.)
,data=cutData,tree=pruned.tree,se=SE,method = "ML")
```
...
p.v.gestlonggest<-signif(p.v.gestlong[3,4], digits = 2)
